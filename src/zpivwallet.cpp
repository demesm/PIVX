...
CzSKUWallet::CzSKUWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zsku seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSKUSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSKUSeed_deprecated()) {
                    LogPrintf("%s: Updated zSKU seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zsku seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zsku wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSKUWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSKUCount(nCountLastUsed);
    else if (!walletdb.ReadZSKUCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSKUWallet::Lock()
{
    seedMaster = 0;
}

void CzSKUWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSKUWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSKU(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSKUWallet::LoadMintPoolFromDB()
{
...
void CzSKUWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSKUWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSKUWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zskuTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSKUWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSKU(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zskuTracker which also adds to database
    pwalletMain->zskuTracker->Add(dMint, true);
...
void CzSKUWallet::SeedToZSKU(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSKUWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSKUWallet::GenerateDeterministicZSKU(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSKUWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSKUWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
