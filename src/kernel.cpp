...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZSkuStake(spend));
    } else {
        ...
        CSkuStake* skuInput = new CSkuStake();
        skuInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(skuInput);
    }
