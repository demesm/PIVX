...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered skucoin address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 SKU.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal skucoin:URI (skucoin:XyZ...?message=example)
            {
...
static void NotifyzSKUReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzSKUReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzSKUReset.connect(boost::bind(NotifyzSKUReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzSKUReset.disconnect(boost::bind(NotifyzSKUReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zskuTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
