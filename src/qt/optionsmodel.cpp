...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::SKU);
    ...
    if (!settings.contains("nAnonymizeSkucoinAmount"))
        settings.setValue("nAnonymizeSkucoinAmount", 1000);

    nAnonymizeSkucoinAmount = settings.value("nAnonymizeSkucoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeSkucoinAmount"))
        SoftSetArg("-anonymizeskucoinamount", settings.value("nAnonymizeSkucoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in skucoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSkucoinAmount:
            return QVariant(nAnonymizeSkucoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSkucoinAmount:
            nAnonymizeSkucoinAmount = value.toInt();
            settings.setValue("nAnonymizeSkucoinAmount", nAnonymizeSkucoinAmount);
            emit anonymizeSkucoinAmountChanged(nAnonymizeSkucoinAmount);
            break;
        case CoinControlFeatures:
