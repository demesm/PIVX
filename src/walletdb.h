...
public:
    ...
    bool WriteZSKUSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSKUSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSKUSeed_deprecated(uint256& seed);
    bool EraseZSKUSeed();
    bool EraseZSKUSeed_deprecated();

    bool WriteZSKUCount(const uint32_t& nCount);
    bool ReadZSKUCount(uint32_t& nCount);
